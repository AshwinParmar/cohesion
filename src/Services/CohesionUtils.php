...
class CohesionUtils {
  ...
  /**
   * Returns a collection of url for the frontend app to be used
   *
   * @param $type string - Whether to return all urls needed for the frontend app
   * or only for the admin or frontend of the website (all,frontend,admin)
   *
   */
  public function jsAppUrlsCollection($type = 'all') {
    $collection = [];
    if($type == 'all' || $type == 'frontend') {
      $collection['frontend-builder-save'] = [
        'url' => Url::fromRoute('cohesion.frontend-builder.save')->toString(),
        'method' => 'POST'
      ];

      $lib = \Drupal::service('library.discovery')->getLibraryByName('cohesion','cohesion-frontend-edit-scripts');
      if(isset($lib['js'][0]['data'])) {
        $collection['frontend-builder-js'] = [
